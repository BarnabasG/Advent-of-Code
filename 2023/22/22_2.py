...
def calculate_falling_blocks(supports):
    falling_blocks = {}
    for b in supports:
        destroyed = {b}
        while True:
            ...
            for s in supports:
                if s in destroyed or len(supports[s]) == 0:
                    continue
                if set(supports[s]).issubset(destroyed):
                    destroyed.add(s)
                    done = False
            if done: break
        falling_blocks[b] = len(destroyed)-1
...
def get_answer(filename='input.txt'):
    ...
    falling_blocks = calculate_falling_blocks(supports)
    return sum(falling_blocks.values())
